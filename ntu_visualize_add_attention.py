...
def visualize(args):
    ...
    def animate(skeleton):
        ...
        for i in range(num_joints):
            for j in range(num_joints):
                if attention_weights[i][j]:
                    joint_locs = skeleton[:,[i,j]]
                    ax.plot(joint_locs[0],joint_locs[1],joint_locs[2], color='gray', linestyle='dashed', linewidth=1)
        ...
        if args.save_interval !=-1:
            if skeleton_index[0] % args.save_interval in [0]:
    ...
    for index in args.indices:
        ...
        ani = FuncAnimation(fig, animate, skeleton_frames) # [T, C, V]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--head', type=int, default=0)  #  [head, head+1] for stgat
